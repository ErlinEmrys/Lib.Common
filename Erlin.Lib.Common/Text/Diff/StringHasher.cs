...
#region Crc32 Code
	...
	/// <summary>
	///    Returns hash of entered text line
	/// </summary>
	/// <param name="line">Text line to hash</param>
	/// <returns>Hash of entered text line</returns>
	public int GetHashCode( string line )
	{
		...
		if( _leadingCharactersToIgnore > 0 )
		{
			line = _leadingCharactersToIgnore >= line.Length ? string.Empty : line[ _leadingCharactersToIgnore.. ];
		}

		if( _ignoreCase )
		{
			line = line.ToUpper( CultureInfo.InvariantCulture );
		}

		switch( _hashType )
		{
			case TextDiffHashType.HashCode:
				return line.GetHashCode();

			case TextDiffHashType.Crc32:
				return StringHasher.GetCrc32( line );

			case TextDiffHashType.Unique:
				return GetUnique( line );

			default:
				throw new CaseNotExpectedException( _hashType );
		}
	}

	/// <summary>
	///    Returns Crc32 hash of text http://www.efg2.com/Lab/Mathematics/CRC.htm
	/// </summary>
	/// <param name="line">Text to get hash</param>
	...
	private static int GetCrc32( string line )
	{
		...
		unchecked
		{
			...
			{
				result = ( result >> 8 ) ^ Crc32Code[ bytes[ i ] ^ ( result & 0xFF ) ];
			}
	...
	/// <summary>
	///    Returns unique hash of text line
	/// </summary>
	/// <param name="line">Text line to get hash</param>
	/// <returns>Unique hash</returns>
	private int GetUnique( string line )
	{
		if( !_uniqueTable.TryGetValue( line, out int result ) )
		{
			result = _uniqueTable.Count + 1;
			_uniqueTable[ line ] = result;
		}
